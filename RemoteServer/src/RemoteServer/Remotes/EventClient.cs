...
namespace RemoteServer.Remotes
{
    ...
    public class EventClient
    {
        ...
        {
            return ConnectAsync(Address, STD_PORT, (uint)System.DateTime.Now.TimeOfDay.Milliseconds);
        }

        public Task ConnectAsync(string Address, int Port)
        {
            return ConnectAsync(Address, Port, (uint)System.DateTime.Now.TimeOfDay.Milliseconds);
        }


        public async Task ConnectAsync(string Address, int Port, uint UID)
        {
            try
            {
                ...
                endPoint = new IPEndPoint((await Dns.GetHostAddressesAsync(Address))[0], Port);
            }
            catch (Exception exc)
            {
                throw new IOException("Failed to connecct", exc);
            }
        }
        ...
        private async Task SendAsync(PacketType PacketType, byte[] Payload)
        {
            ...
            for (int Package = 1; Package <= packetCount; Package++)
            {
                ...
                ArraySegment<byte> bytes = new ArraySegment<byte>(packet, 0, header.Length + bytesToSend);

                int sendSize;
                try
                {
                    sendSize = await socket.SendToAsync(bytes, SocketFlags.None, endPoint);
                }
                catch (Exception exc)
                {
                    throw new IOException("Failed to send packet", exc);
                }

                if (sendSize != (header.Length + bytesToSend))
                {
                    throw new IOException(String.Format("Only send {0} of {1} bytes of event", sendSize, header.Length + bytesToSend));
                }
        ...
        /************************************************************************/
        public Task SendHeloAsync(string DevName, IconType IconType, string IconFile)
        {
            ...
            return SendAsync(PacketType.PT_HELO, payload);

        }

        public Task SendHeloAsync(string DevName)
        {
            return SendHeloAsync(DevName, IconType.ICON_NONE, "");
        }
        ...
        /* XX - imagedata ( can span multiple packets )                         */
        /************************************************************************/
        public Task SendNotificationAsync(string Caption, string Message, IconType IconType, string IconFile)
        {
            ...
            return SendAsync(PacketType.PT_NOTIFICATION, payload);

        }

        public Task SendNotificationAsync(string Caption, string Message)
        {
            return SendNotificationAsync(Caption, Message, IconType.ICON_NONE, "");
        }
        ...
        /************************************************************************/
        private Task SendButtonAsync(string Button, ushort ButtonCode, string DeviceMap, ButtonFlagsType Flags, short Amount)
        {
            ...
            return SendAsync(PacketType.PT_BUTTON, payload);

        }

        public Task SendButtonAsync(string Button, string DeviceMap, ButtonFlagsType Flags, short Amount)
        {
            return SendButtonAsync(Button, 0, DeviceMap, Flags, Amount);
        }

        public Task SendButtonAsync(string Button, string DeviceMap, ButtonFlagsType Flags)
        {
            return SendButtonAsync(Button, 0, DeviceMap, Flags, 0);
        }

        public Task SendButtonAsync(ushort ButtonCode, string DeviceMap, ButtonFlagsType Flags, short Amount)
        {
            return SendButtonAsync("", ButtonCode, DeviceMap, Flags, Amount);
        }

        public Task SendButtonAsync(ushort ButtonCode, string DeviceMap, ButtonFlagsType Flags)
        {
            return SendButtonAsync("", ButtonCode, DeviceMap, Flags, 0);
        }

        public Task SendButtonAsync(ushort ButtonCode, ButtonFlagsType Flags, short Amount)
        {
            return SendButtonAsync("", ButtonCode, "", Flags, Amount);
        }

        public Task SendButtonAsync(ushort ButtonCode, ButtonFlagsType Flags)
        {
            return SendButtonAsync("", ButtonCode, "", Flags, 0);
        }

        public Task SendButtonAsync()
        {
            return SendButtonAsync("", 0, "", ButtonFlagsType.BTN_UP, 0);
        }
        ...
        public Task SendPing()
        {
            ...
            return SendAsync(PacketType.PT_PING, payload);
        }
        ...
        {
            byte[] payload = new byte[0];
            return SendAsync(PacketType.PT_BYE, payload);
        }
        ...
        /************************************************************************/
        public Task SendMouseAsync(ushort X, ushort Y, MouseFlagsType Flags)
        {
            ...
            return SendAsync(PacketType.PT_MOUSE, payload);

        }

        public Task SendMouseAsync(ushort X, ushort Y)
        {
            return SendMouseAsync(X, Y, MouseFlagsType.MS_ABSOLUTE);
        }
        ...
        /* %s - message                                                         */
        /************************************************************************/
        public Task SendLogAsync(LogTypeEnum LogLevel, string Message)
        {
            ...
            return SendAsync(PacketType.PT_LOG, payload);
        ...
        /* %s - action message                                                  */
        /************************************************************************/
        public Task SendActionAsync(ActionType Action, string Message)
        {
            ...
            return SendAsync(PacketType.PT_ACTION, payload);

        }

        public Task SendActionAsync(string Message)
        {
            return SendActionAsync(ActionType.ACTION_EXECBUILTIN, Message);
        }
